...
/// A widget that can be used to create a generic UI with.
...
/// A list of scrollable widgets for every parameter in a [`Params`] object.
...
impl<'a, W: ParamWidget> GenericUi<'a, W> {
    /// Convert this [`GenericUi`] into an [`Element`] with the correct message.
