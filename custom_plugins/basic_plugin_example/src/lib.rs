...
const GUI_PUBLISHED_URL: &str = "https://basic-plugin-example-web-gui.vercel.app";
...
fn webview_userdata_dir() -> PathBuf {
    if let Some(proj) = ProjectDirs::from("com", "WTH Plugins", "BasicPluginExample") {
        proj.data_local_dir().join("webview_userdata")
    } else {
        std::env::temp_dir().join("basic_plugin_example_webview_userdata")
    }
}
