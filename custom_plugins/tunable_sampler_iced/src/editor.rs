...
use crate::TunableSamplerIcedParams;
...
pub(crate) fn create(
    ...
    create_iced_editor::<TunableSamplerIcedEditor>(editor_state, (params, peak_meter))
}

struct TunableSamplerIcedEditor {
    params: Arc<TunableSamplerIcedParams>,
...
impl IcedEditor for TunableSamplerIcedEditor {
    type Executor = executor::Default;
    type Message = Message;
    type InitializationFlags = (Arc<TunableSamplerIcedParams>, Arc<AtomicF32>);

    fn new(
        (params, peak_meter): Self::InitializationFlags,
        context: Arc<dyn GuiContext>,
    ) -> (Self, Command<Self::Message>) {
        let editor = TunableSamplerIcedEditor {
            params,
            context,
            peak_meter,
