const MU: f64 = 0.003;
const TENSION: f64 = 40.0;
const SPRING_K: f64 = 30_000.0;
const EI: f64 = 3e-8;
const INTERIOR_DAMP: f64 = 0.25;
const EP_DAMP_BASE: f64 = 0.75;
const EP_TAPER_COUNT: usize = 3;
const PLUCK_FRACTION: f64 = 1.0 / 3.0;
const PLUCK_AMPLITUDE: f64 = 0.001;
const PICKUP_FRACTION: f64 = 0.15;
const OUTPUT_GAIN: f64 = 2.5;
...
impl StringPhysics {
    pub fn new(sample_rate: f32) -> Self {
        ...
        let n_total = (l_max / MAX_SEG_LENGTH).ceil() as usize + 1;
        let n_segs = n_total - 1;
        let seg_len = l_max / n_segs as f64;
        let node_mass = MU * seg_len;
        let bfc = EI / (seg_len * seg_len * seg_len);
        let dt = 1.0 / sr;
        let effective_end = n_total - 2;
        let pickup_index = (PICKUP_FRACTION * effective_end as f64).round() as usize;
        ...
        Self {
            ...
            spring_k: SPRING_K,
            interior_damping: INTERIOR_DAMP,
            endpoint_damp_base: EP_DAMP_BASE,
    ...
    pub fn set_pitch(&mut self, freq_hz: f64) {
        self.effective_end = freq_to_effective_end(freq_hz, self.c_wave, self.seg_len, self.n_total);
        self.pickup_index = (PICKUP_FRACTION * self.effective_end as f64).round() as usize;
    ...
    pub fn pluck(&mut self, velocity: u8) {
        ...
        let apex = ((PLUCK_FRACTION * self.effective_end as f64).round() as usize)
    ...
    pub fn step(&mut self) {
        ...
        // Step 1: accelerations at current state
        ...
        // Step 3: accelerations at predicted state
        compute_accelerations(
    ...
    pub fn output(&self) -> f32 {
        (self.vy[self.pickup_index] * OUTPUT_GAIN) as f32
    }
}
...
fn compute_accelerations(
    ...
    // Bending stiffness (5-point biharmonic stencil)
    if effective_end >= 4 {
    ...
    // Endpoint damping (tapered over ep_taper_count nodes near each pinned end)
