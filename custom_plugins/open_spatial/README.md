...
Open Spatial is a minimal saturation effect (clip(sat * input) * gain) with a webview-based GUI. It is meant to serve as a practical starter template for new plugins.
...
at `https://wth-plugins-open-spatial.vercel.app`.

Configure the Vite dev server to rewrite `/wth-plugin-name` to
`/open_spatial` so the plugin only connects to the intended app.
